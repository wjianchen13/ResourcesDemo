...
public enum BitmapType {
	NinePatch {
		@Override
		public NinePatchChunk1 createChunk(Bitmap bitmap) {
			return NinePatchChunk1.parse(bitmap.getNinePatchChunk());
		}
	}, RawNinePatch {
		@Override
		protected NinePatchChunk1 createChunk(Bitmap bitmap) {
			try {
				return NinePatchChunk1.createChunkFromRawBitmap(bitmap, false);
			} catch (WrongPaddingException e) {
				return NinePatchChunk1.createEmptyChunk();
		...
		@Override
		protected Bitmap modifyBitmap(Resources resources, Bitmap bitmap, NinePatchChunk1 chunk) {
	...
	public static BitmapType determineBitmapType(Bitmap bitmap) {
		...
		if (ninePatchChunk != null && android.graphics.NinePatch.isNinePatchChunk(ninePatchChunk))
			return NinePatch;
		if (NinePatchChunk1.isRawNinePatchBitmap(bitmap))
			return RawNinePatch;
		return PlainImage;
	}
	...
	protected NinePatchDrawable createNinePatchDrawable(Resources resources, Bitmap bitmap, String srcName) {
		NinePatchChunk1 chunk = createChunk(bitmap);
